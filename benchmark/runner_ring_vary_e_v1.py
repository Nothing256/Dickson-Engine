...
DICKSON_CLI = "build/bin/dickson_cli"
RESULTS_DIR = "results"
...
def main():
    ...
    with open(os.path.join(RESULTS_DIR, "log_runner_ring_vary_e_v1.txt"), "w") as log:
        log.write(f"=== V1 Ring Lifting: vary e, fixed p={p} ===\n")
    ...
    with open(os.path.join(RESULTS_DIR, "ring_vary_e_v1.json"), "w") as f:
        json.dump(results, f, indent=4)
        
    print("[Done] Saved to ring_vary_e_v1.json")

if __name__ == "__main__":
    if not os.path.exists(DICKSON_CLI):
        print(f"Error: {DICKSON_CLI} not found. Please compile.")
    else:
        main()
